...
syms Lambda_R Lambda_UAV Lambda_DL Lambda_LL %Define random variables of limit state function ****

DL_LL_Ratio = 0.2; %DL/LL Ratio to solve Limit State function for

g = Lambda_R*Lambda_UAV*((1.25*DL_LL_Ratio+1.75)/0.9)-(Lambda_DL*DL_LL_Ratio+Lambda_LL); %Limit state function ****

Variables_For_Input = [Lambda_DL; Lambda_LL; Lambda_UAV; Lambda_R]; %Random Variable Vector of Limit State Function (Last variable is does not require a Initial Design Point[x*])****
...
Initial_Design_Points_For_Input = [Initial_Design_Point_Lambda_DL; Initial_Design_Point_Lambda_LL; Initial_Design_Point_Lambda_UAV]; %Vector of Initial Design Points (x*) [None required for last variable listed if Variables vector] ****
...
Lambda_UAV_Mew_Input = 0.0037709;
Lambda_UAV_Sigma_Input = 0.2548168;
[Equivalent_Mean_Lambda_UAV, Equivalent_SD_Lambda_UAV] = Lognormal_Equivalent_Normal_Mean_and_SD_SUBF(Initial_Design_Points_For_Input(3), Lambda_UAV_Mew_Input, Lambda_UAV_Sigma_Input); %First Equivalent inital values for Lambda_UAV



%Last variable input values (Subfunction is in the a=1 part of loop) 
Lambda_R_Mew_Input = -0.019610;
Lambda_R_Sigma_Input = 0.198042;



%All Equivelent values except for the ones not yet calculated 
Equivelent_Means_For_Input = [Equivalent_Mean_Lambda_DL; Equivalent_Mean_Lambda_LL; Equivalent_Mean_Lambda_UAV]; %Vector of equivelent Means ****
Equivelent_SDs_For_Input = [Equivalent_SD_Lambda_DL; Equivalent_SD_Lambda_LL; Equivalent_SD_Lambda_UAV]; %Vector of equivelent Standard Deviations ****
...
for a = 1:Number_of_Itterations+1
    ...
    elseif a == 1 %If on first itteration of the loop
        ...
        Equivelent_Mean_Storage(3,a) = Equivalent_Mean_Lambda_UAV; %Store Lambda_UAV Equivelent Mean
        Equivelent_SD_Storage(3,a) = Equivalent_SD_Lambda_UAV; %Store Lambda_UAV Equivelent SD

        Equivelent_Mean_Storage(4,a) = Equivalent_Mean_Lambda_R; %Store Lambda_R Equivelent Mean
        Equivelent_SD_Storage(4,a) = Equivalent_SD_Lambda_R; %Store Lambda_R Equivelent SD
        ...
        [Equivalent_Mean_Lambda_UAV, Equivalent_SD_Lambda_UAV] = Lognormal_Equivalent_Normal_Mean_and_SD_SUBF(Old_Design_Points(3), Lambda_UAV_Mew_Input, Lambda_UAV_Sigma_Input); %Equivalent values for Lambda_UAV
        Equivelent_Mean_Storage(3,a) = Equivalent_Mean_Lambda_UAV; %Store Lambda_UAV Equivelent Mean
        Equivelent_SD_Storage(3,a) = Equivalent_SD_Lambda_UAV; %Store Lambda_UAV Equivelent SD


        [Equivalent_Mean_Lambda_R, Equivalent_SD_Lambda_R] = Lognormal_Equivalent_Normal_Mean_and_SD_SUBF(Old_Design_Points(4),Lambda_R_Mew_Input,Lambda_R_Sigma_Input); %Equivalent values for Lambda_R
        Equivelent_Mean_Storage(4,a) = Equivalent_Mean_Lambda_R; %Store Lambda_R Equivelent Mean
        Equivelent_SD_Storage(4,a) = Equivalent_SD_Lambda_R; %Store Lambda_R Equivelent SD
        ...
        Equivelent_Means_For_Input = [Equivalent_Mean_Lambda_DL; Equivalent_Mean_Lambda_LL; Equivalent_Mean_Lambda_UAV; Equivalent_Mean_Lambda_R]; %Vector of equivelent Means ****
        Equivelent_SDs_For_Input = [Equivalent_SD_Lambda_DL; Equivalent_SD_Lambda_LL; Equivalent_SD_Lambda_UAV; Equivalent_SD_Lambda_R;]; %Vector of equivelent Standard Deviations ****

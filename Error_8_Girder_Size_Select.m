function [Controling_Graph,plf_Controling,WShape_High_Bound_Controling,WShape_Controling] = Error_8_Girder_Size_Select(Total_Span_Length_Input,Numb_Spans_Input,Numb_Girders_Input,Bridge_Width_Input,Girder_Depth_Input)
%Subfunction Version of Girder_Size_Calculations_For_CSIBridge_Models.m
    ...
    %Calculate Plf for each graph
    for i = 1:length(WShape_Controling) %For each graph type (1 = all spacing, 2 = specified spacing 1, 3 = specified spacing 2)
        Error_Value = WShape_Controling(i); %Error value
        if Error_Value == -1 %if a -1 error value
            %Use minimum graph span length for calculation (50ft)
            [Temp_Controling_Graph(i),Temp_plf_Controling(i),Temp_WShape_High_Bound_Controling{i},Temp_WShape_Controling{i}] = Girder_Size_Select(50*Numb_Spans,Numb_Spans,Numb_Girders,Bridge_Width,Girder_Depth); %Calculate with controling span length of 50 ft
            
        elseif Error_Value == -2 %if a -2 error value
            %Use max graph span length for calculation (350 or 450 ft)
            if Numb_Spans == 1 %For 1 span graphs
                [Temp_Controling_Graph{i},Temp_plf_Controling(i),Temp_WShape_High_Bound_Controling{i},Temp_WShape_Controling{i}] = Girder_Size_Select(350,Numb_Spans,Numb_Girders,Bridge_Width,Girder_Depth); %Calculate with controling span length of 350 ft
            elseif Numb_Spans == 2 %For 2 span graph
                [Temp_Controling_Graph{i},Temp_plf_Controling(i),Temp_WShape_High_Bound_Controling{i},Temp_WShape_Controling{i}] = Girder_Size_Select(350,Numb_Spans,Numb_Girders,Bridge_Width,Girder_Depth); %Calculate with controling span length of 350 ft
            else %For 3 or more spans
                [Temp_Controling_Graph{i},Temp_plf_Controling(i),Temp_WShape_High_Bound_Controling{i},Temp_WShape_Controling{i}] = Girder_Size_Select(450,Numb_Spans,Numb_Girders,Bridge_Width,Girder_Depth); %Calculate with controling span length of 450 ft
            end

        elseif Error_Value == -3 %if a -3 error value (Is ok, all spacing grph will always have a value)
            Temp_Controling_Graph{i} = 'None';
            Temp_plf_Controling(i) = NaN;
            Temp_WShape_High_Bound_Controling{i} = 'None';
            Temp_WShape_Controling{i} = 'None';
        
        elseif Error_Value == 0 %if graph does not exist 
            Temp_Controling_Graph{i} = 'None';
            Temp_plf_Controling(i) = NaN;
            Temp_WShape_High_Bound_Controling{i} = 'None';
            Temp_WShape_Controling{i} = 'None';
    ...
    %Find controling plf
    Contriling_Index = find(Temp_plf_Controling == min(Temp_plf_Controling)); %Find index locations of controling plf
    
    Single_Index = min(Contriling_Index); %Gets index location if there is more than one index value (Defalts to All Spacing graph)
   
    
    Controling_Graph = Temp_Controling_Graph(Single_Index);
    plf_Controling = Temp_plf_Controling(Single_Index);
    WShape_High_Bound_Controling =  Temp_WShape_High_Bound_Controling(Single_Index);
    WShape_Controling = Temp_WShape_Controling(Single_Index);
